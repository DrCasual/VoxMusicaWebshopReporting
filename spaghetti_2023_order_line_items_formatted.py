...
def main():
    ...
    # read in the data
    df_order_line_items = pd.read_sql_query("SELECT * FROM order_line_items WHERE product_id IN (247, 234, 238)", con)
    
    # transform data
    df_order_line_items = format_line_items(df_order_line_items)
    
    # write data to table
    df_order_line_items.to_sql("spaghetti_2023_order_line_items_formatted", con, index=False, if_exists='replace')
